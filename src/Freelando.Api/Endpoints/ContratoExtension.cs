...
public static class ContratoExtensions
{
    public static void AddEndPointContratos(this WebApplication app)
    {
        ...
        {
            var contrato = converter.EntityListToResponseList(await unitOfWork.ContratoRepository.GetAll());
        ...
        {
            var contrato = await unitOfWork.ContratoRepository.GetById(x => x.Id == id);
            if (contrato is null)
            {
            ...
            await unitOfWork.ContratoRepository.Update(contrato);
        ...
        app.MapDelete("/contrato/{id}", async ([FromServices] ContratoConverter converter, [FromServices] IUnitOfWork unitOfWork, Guid id) =>
        {
            ...
            await unitOfWork.ContratoRepository.Delete(contrato);
            await unitOfWork.Commit();
